...
export default function Referral() {
  ...
  // ✅ QR uses current origin (prevents 404 due to wrong domain)
  const referralUrl = useMemo(() => `${window.location.origin}/referral`, []);
  const intakePdfUrl = useMemo(() => "/forms/referral-page.pdf", []);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (file instanceof File && file.size > 0) {
      if (file.size > MAX_FILE_BYTES) {
        setAttachmentError(`Attachment is too large. Max ${MAX_FILE_MB}MB.`);
        focusStatus();
        return;
      }
      if (!ALLOWED_MIME.has(file.type)) {
        setAttachmentError("Invalid file type. Please upload PDF, JPG, or PNG only.");
        focusStatus();
        return;
      }
    }
    ...
    try {
      // ✅ Send multipart/form-data (matches your updated PHP)
      const fd = new FormData();
      fd.set("referrerName", payload.referrerName);
      fd.set("referrerEmail", payload.referrerEmail);
      fd.set("referrerPhone", payload.referrerPhone);
      fd.set("participantName", payload.participantName);
      fd.set("participantPhone", payload.participantPhone);
      fd.set("participantEmail", payload.participantEmail);
      fd.set("referralType", payload.referralType);
      fd.set("message", payload.message);
      fd.set("consent", payload.consent ? "yes" : "");
      fd.set("csrf", csrf);
      fd.set("turnstileToken", turnstileToken);
      fd.set("company", "");

      // attach file only if chosen
      if (file instanceof File && file.size > 0) {
        fd.set("attachment", file, file.name);
      }
      ...
      <div className="ref-grid">
        ...
        <section className="ref-card" aria-label="Referral help">
          ...
          <div className="qr-block" aria-label="Referral QR code" style={{ marginTop: "1rem" }}>
            ...
            <div className="cta-row" style={{ marginTop: ".5rem", flexWrap: "wrap" }}>
              <button type="button" className="btn ghost" onClick={downloadQrPng}>
                Download QR (PNG)
              </button>

              <a className="btn ghost" href={intakePdfUrl} target="_blank" rel="noreferrer">
                Open intake form (PDF)
              </a>

              <a className="btn ghost" href={intakePdfUrl} download>
        ...
        <section className="ref-card" aria-label="Referral form">
          ...
          <div
            ...
            role="status"
          >
            {(errors.length > 0 || attachmentError) && (
              <ul className="error-list form-pop">
                {errors.map((er, i) => (
                  <li key={`e-${i}`}>{er}</li>
                ))}
                {attachmentError && <li>{attachmentError}</li>}
              </ul>
            )}
          ...
          <form className="ref-form" onSubmit={handleSubmit} noValidate>
            ...
            <label>
              ...
              />
              <span className="muted" style={{ display: "block", marginTop: ".25rem" }}>
                Optional. PDF/JPG/PNG only. Max {MAX_FILE_MB}MB.
              </span>
            </label>
